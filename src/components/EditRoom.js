...
function EditFloor(props){
    ...
    const readMTR = async() => {
        ...
        .then(function (response){
            ...
            try{
                room = meetingRoomData[props.selectedLocation][props.selectedFloor]
            ...
            for(let i = 0; i < room.length; i++){
                itemList.push({index: i, text:room[i]})
            }

            const addeditem = addedList[props.selectedFloor]
            if (addeditem !== undefined){
                for(let i = 0; i < addeditem.length; i++){
                    itemList.push({index: room.length+i, text:addeditem[i]})
                }
            }
            setItems(itemList)
      ...
      useEffect(() => {
        readMTR(addedList)
      }, [props.selectedLocation, props.selectedFloor])
    ...
    const onClickBtn = () => {
        ...
        if (!(props.selectedFloor in temp)){temp[props.selectedFloor]=[]}
        temp[props.selectedFloor].push(inputText)
        setAddedList(temp)
        setInputText('');
    }
